...
class Rbac {
    ...
    static public function AccessDecision($appName=APP_NAME) {
        ...
        if(RBAC::checkAccess()) {
            ...
            if(!session(config('ADMIN_AUTH_KEY'))) {
                if(config('USER_AUTH_TYPE')==2) {
                    //加强验证和即时验证模式 更加安全 后台权限修改可以即时生效
                    //通过数据库进行访问检查
                    $accessList = RBAC::getAccessList(session(config('USER_AUTH_KEY')));
                }else {
                ...
                if(!isset($accessList[strtoupper($appName)][strtoupper($module)][strtoupper(Request::instance()->action())])) {
